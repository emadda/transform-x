// Use esbuild binary loader to package .wasm file with JS into a single importable ES module.
import sqlite_wasm_binary from "./../../../node_modules/@sqlite.org/sqlite-wasm/sqlite-wasm/jswasm/sqlite3.wasm";

// When: Browser: `self` is `window` which has `BigInt` and other objects. (-bundler builds are only designed for the web browser, but the API's are likely all supported between Web and Node as they both use V8).
// When: Node: `self` is undefined, so it must be set.
const self = self ?? global;
...
var sqlite3InitModule = (() => {
  ...
  return function (config) {
    ...
    if (Module['locateFile']) {
      ...
      // Use esbuild binary loader to package .wasm file with JS into a single importable ES module.
      wasmBinaryFile = URL.createObjectURL(new Blob([sqlite_wasm_binary.buffer], {type: "application/wasm"}));
    }
